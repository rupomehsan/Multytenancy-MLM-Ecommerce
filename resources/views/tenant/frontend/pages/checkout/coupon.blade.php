...
<div class="checkout-order-review-coupon-box">
    ...
    <div class="cart-single-coupon-form">
        <div class="cart-single-coupon-input">
            <input type="text" placeholder="Enter coupon" name="coupon_code" style="padding-left: 10px;"
        ...
        @if ($showMinimumOrderWarning)
             <small class="mt-2 d-block {{ $meetsMinimumOrder ? 'text-success' : 'text-warning' }}" id="coupon-minimum-warning">
                @if ($meetsMinimumOrder)
                    ✓ Minimum order requirement met ({{ number_format($minimumOrderAmount, 2) }} BDT)
                @else
                    Coupon requires minimum order of {{ number_format($minimumOrderAmount, 2) }} BDT (Current: {{ number_format($subTotal, 2) }} BDT)
                @endif
            </small>
        @endif
    </div>
