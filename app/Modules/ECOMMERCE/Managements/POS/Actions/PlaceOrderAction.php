...
class PlaceOrderAction
{
    ...
    private static function calculateOrderTotals(Request $request): array
    {
        ...
        // Do not apply delivery cost for store pickup orders
        $deliveryCost = 0;
        if (isset($request->delivery_method) && $request->delivery_method == Order::DELIVERY_STORE_PICKUP) {
            $deliveryCost = 0;
        } else {
    ...
    {
        // Ensure delivery fee is zero for store pickup regardless of submitted values
        $deliveryFeeToSave = $calculations['delivery_cost'];
        $totalToSave = $calculations['grand_total_without_round_off'];
        if (isset($request->delivery_method) && $request->delivery_method == Order::DELIVERY_STORE_PICKUP) {
            $deliveryFeeToSave = 0;
            // remove previously included delivery cost from total
            $totalToSave = $totalToSave - $calculations['delivery_cost'];
            if ($totalToSave < 0) {
                $totalToSave = 0;
            }
        }
